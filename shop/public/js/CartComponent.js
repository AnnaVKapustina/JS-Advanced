Vue.component('cart', {
    ...
    template: `<div>
    <button type="button" class="header__menu__icon header__menu__icon__cart" @click="showCart = !showCart">
                                ...
                                </svg><span class="header__menu__icon__number">{{ this.cartItems.reduce((summ, item) => summ + item.quantity, 0) }}</span>
                            </button>          
            <div class="cart-block" v-show="showCart"> 
            <h2 v-if="!cartItems.length">Cart is empty</h2> 
            <div v-else >    
                <h2> {{ this.cartItems.reduce((summ, item) => summ + item.quantity, 0) }} item(s) </br>
                TOTAL SUM:{{ this.cartItems.reduce((summ, item) => summ + item.quantity * item.price, 0) }} $ </h2>
                <cart-item v-for="item of cartItems" :img="item.imgPath" :cart-item="item" :cart-count = "cartCount"
...
Vue.component('cart-item', {
    ...
    template: `<div class="cart-item">
        ...
        <div class="product-desc">
            ...
            <p>SUM: {{ cartItem.price * cartItem.quantity }} $</p>
        </div>
    </div>`
})